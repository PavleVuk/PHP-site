...
if (empty($model)) { array_push($greske, "Naziv modela je obavezan"); }
  ...
  if ($telefon) { 
    if ($telefon['IMEI'] === $IMEI) {
      array_push($greske, "Telefon već postoji");
    }
}
if (count($greske) == 0) {
  	$unos = "INSERT INTO mobilni_telefon (IMEI, Proizvodjac, Model, JMBG_korisnika) 
  			  VALUES('$IMEI','$proizvodjac', '$model', '$JMBG_korisnika')";
...
<center>
<div class="reg">
<h1> Unesite podatke mobilnog telefona </h1>
<form name="registracija" class="regi" method="post" action="unos.php" >
<p> Unesite IMEI : <br> <input type="text" size="14" name="IMEI" value="<?php echo $IMEI;?>"> </p>
<p> Unesite proizvođača: <br><input type="text" size="14" name="proizvodjac" value="<?php echo $proizvodjac;?>"> </p>
<p> Unesite model: <br><input type="text" size="15" name="model" value="<?php echo $model;?>" > </p>
